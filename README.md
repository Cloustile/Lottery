...
一个功能完整的 Web 抽奖应用，支持身份证号和手机号两种登录方式，每个人只能抽一次奖。

## ✨ 功能特性

### 用户端功能
- ✅ **双模式登录**：支持身份证号或手机号 + 验证码登录
...
1. **选择登录方式**
   - 身份证号：输入 18 位身份证号和姓名
   - 手机验证：输入 11 位手机号，获取验证码（测试码：123456），输入验证码和姓名
...
1. **验证码**：当前为测试模式，固定验证码为 `123456`
   - 生产环境请对接短信服务商（如阿里云、腾讯云短信服务）

2. **身份证号验证**：当前仅验证长度
   - 可根据需要添加更严格的校验规则

3. **会话管理**：使用 Flask session 存储用户状态
